...
import { buildImpactDecision, buildRiskAlert, calculateHazardAtHour, calculateWeatherRisk } from '@/lib/riskEngine';
...
export async function fetchRiskAlertAssessment(location, persona = 'citizen', timezone = 'auto') {
  const params = new URLSearchParams({
    latitude: String(location.latitude), longitude: String(location.longitude),
    current: 'temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m',
    hourly: 'precipitation,precipitation_probability,temperature_2m,apparent_temperature,wind_speed_10m',
    forecast_days: '2', timezone,
  });
  const response = await fetch(`https://api.open-meteo.com/v1/forecast?${params}`);
  if (!response.ok) throw new Error(`Open-Meteo risk HTTP ${response.status}`);
  ...
  const hourly = (data.hourly?.time || []).map((time, index) => ({
    time,
    precipitation: numberFrom(data.hourly.precipitation?.[index]),
    precipitationProbability: numberFrom(data.hourly.precipitation_probability?.[index]),
    temperature: numberFrom(data.hourly.temperature_2m?.[index]),
    apparentTemperature: numberFrom(data.hourly.apparent_temperature?.[index]),
    windSpeed: numberFrom(data.hourly.wind_speed_10m?.[index]),
  }));
  const summary = calculateWeatherRisk({ current, hourly });
  const highest = [...summary.hazards].sort((a, b) => b.score - a.score)[0];
  const firstHour = hourly.find((hour) => new Date(hour.time) >= new Date()) || hourly[0] || {};
  const selected = calculateHazardAtHour({ current, hour: firstHour, hazard: highest.type });
  const timeline = summary.timelineByHazard?.[highest.type] || [];
  const peakRisk = timeline.reduce((best, point) => !best || point.score > best.score ? point : best, null);
  const impactDecision = buildImpactDecision({ hazard: highest.type, score: selected.score, level: selected.level, peakRisk, persona });
  return buildRiskAlert({ area: location.city || 'selected area', hazard: highest.type, score: selected.score, level: selected.level, factors: selected.factors, peakRisk, impactDecision, persona, assessedAt: new Date().toISOString() });
}
...
export function buildRiskAlertPayload(alert, timezone = 'Asia/Kolkata') {
  const when = alert.expectedTime ? formatLocalTime(new Date(alert.expectedTime), timezone) : 'now';
  const drivers = alert.reason || 'Live weather signals indicate elevated risk.';
  const impact = alert.impact || 'Weather-related disruption is possible.';
  return {
    title: `${alert.alertLevel} ${alert.hazard} risk - WeatherGPT`,
    body: `${alert.area}: ${alert.timing} ${alert.alertLevel.toLowerCase()} risk around ${when}. ${alert.escalation ? `${alert.escalation} ` : ''}${drivers} Impact: ${impact} Action: ${alert.recommendedAction}`,
    icon: '/icons/weatherGPT logo.png', badge: '/icons/weatherGPT logo.png',
    tag: `weathergpt-risk-${alert.riskSignature}`,
    data: { url: '/risk', type: 'risk-intelligence', alert },
    actions: [{ action: 'view-risk', title: 'View Risk Map' }, { action: 'dismiss', title: 'Dismiss' }],
  };
}
