...
export function cleanTextForSpeech(rawText, language = 'english') {
  ...
  if (language === 'bengali') {
    ...
    text = text.replace(/(\d+)\s*°C/g, '$1 ડિગ્રી સેલ્સિયસ');
    text = text.replace(/(\d+)\s*°/g, '$1 ડિગ્રી');
    text = text.replace(/(\d+)\s*%/g, '$1 ટકા');
    text = text.replace(/(\d+)\s*(?:કિમી\/કલાક|km\/h|kmph)/gi, '$1 કિલોમીટર પ્રતિ કલાક');
    text = text.replace(/\bUV\s*:\s*/gi, 'યુવી ઈન્ડેક્સ ');
  } else {
    // English
