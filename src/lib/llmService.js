...
function generateSmartLocalResponseRaw(persona, language, weather, userQuery, forecast, extraContext = {}) {
  const w = weather || {
    city: 'your location',
    temp: 28,
    feelsLike: 30,
  ...
  const risk = extraContext.risk;
  const decision = extraContext.decision || risk?.impactDecision?.decision;
  const impacts = extraContext.impact || risk?.impactDecision?.impacts || [];
  const incidents = extraContext.incidents?.incidents || (Array.isArray(extraContext.incidents) ? extraContext.incidents : []);
  const incidentAdvisory = extraContext.incidents?.advisory || extraContext.groundReality?.advisory;
  const peakTime = risk?.peakRisk?.time ? new Date(risk.peakRisk.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : null;
  const riskLevel = risk?.level || 'low';
  const riskType = risk?.type || risk?.hazard || 'weather';
  const isElevatedRisk = riskLevel === 'high' || riskLevel === 'severe';
  ...
  // Check if query is asking for travel, outdoor safety, work advice, or if elevated risk exists
  const isSafetyOrTravelQuery = (
    q.includes('travel') || q.includes('tonight') || q.includes('safe') ||
    q.includes('bahar') || q.includes('jaana') || q.includes('nikal') ||
    q.includes('delivery') || q.includes('reroute') || q.includes('route') ||
    q.includes('work') || q.includes('pour') || q.includes('crane') ||
    q.includes('harvest') || q.includes('irrigate') || q.includes('spray') ||
    q.includes('risk') || q.includes('flood') || q.includes('alert')
  );

  // If structured risk is elevated OR user is asking safety/travel advice, provide grounded pipeline explanation
  if (risk && (isElevatedRisk || isSafetyOrTravelQuery)) {
    const directAns = isElevatedRisk
      ? (resolvedPersona === 'logistics'
          ? `⛔ **Rerouting strongly recommended for ${w.city}.**`
          : resolvedPersona === 'farmer'
            ? `🌾 **Hold field operations in ${w.city}.**`
            : resolvedPersona === 'construction'
              ? `⚠️ **Pause exposed outdoor and crane operations.**`
              : resolvedPersona === 'authority'
                ? `🚨 **Elevate civic monitoring and pump deployment.**`
                : `⛔ **Non-essential travel tonight is NOT recommended in ${w.city}.**`)
      : `✅ **Conditions are currently manageable in ${w.city}.**`;

    const driversList = (risk.factors || risk.drivers || [])
      .filter((f) => f.label && f.value)
      .map((f) => `• ${f.label}: ${f.value}`)
      .join('\n') || `• Precipitation & humidity signals indicate ${riskLevel} risk.`;

    const impactText = impacts.length > 0
      ? impacts.map((imp) => `• ${imp}`).join('\n')
      : '• Localized weather disruptions possible during peak window.';

    const recAction = decision?.recommendations?.[0] || 'Monitor live updates and official guidance.';
    const incidentNotice = incidentAdvisory?.text
      ? `\n\n📍 **Ground Reality:** ${incidentAdvisory.text}`
      : incidents.length > 0
        ? `\n\n📍 **Ground Reality:** ${incidents[0].category?.replace('_', ' ')} reported ${incidents[0].distanceKm} km away (${incidents[0].verification}).`
        : '';

    if (language === 'hindi') {
      return `${directAns}\n\n⚠️ **जोखिम स्तर:** ${riskLevel === 'severe' ? 'गंभीर' : 'उच्च'} ${riskType === 'flood' ? 'बाढ़' : riskType} जोखिम\n\n🔍 **कारण (Risk Drivers):**\n${driversList}\n\n💥 **संभावित प्रभाव (Impact):**\n${impactText}${peakTime ? `\n\n⏱️ **चरम समय (Peak Window):** लगभग ${peakTime}` : ''}\n\n📋 **सलाह (${roleConfig.label}):** ${recAction}${incidentNotice}`;
    }

    if (language === 'bengali') {
      return `${directAns}\n\n⚠️ **ঝুঁকির মাত্রা:** ${riskLevel === 'severe' ? 'মারাত্মক' : 'উচ্চ'} ${riskType === 'flood' ? 'বন্যা' : riskType} ঝুঁকি\n\n🔍 **কারণ (Risk Drivers):**\n${driversList}\n\n💥 **সম্ভাব্য প্রভাব (Impact):**\n${impactText}${peakTime ? `\n\n⏱️ **সর্বোচ্চ সময় (Peak Window):** আনুমানিক ${peakTime}` : ''}\n\n📋 **পরামর্শ (${roleConfig.label}):** ${recAction}${incidentNotice}`;
    }

    if (language === 'gujarati') {
      return `${directAns}\n\n⚠️ **જોખમ સ્તર:** ${riskLevel === 'severe' ? 'ગંભીર' : 'ઉચ્ચ'} ${riskType === 'flood' ? 'પૂર' : riskType} જોખમ\n\n🔍 **મુખ્ય કારણો (Risk Drivers):**\n${driversList}\n\n💥 **સંભવિત અસરો (Impact):**\n${impactText}${peakTime ? `\n\n⏱️ **પીક સમય (Peak Window):** આશરે ${peakTime}` : ''}\n\n📋 **ભલામણ કરેલ પગલાં (${roleConfig.label}):** ${recAction}${incidentNotice}`;
    }

    return `${directAns}\n\n⚠️ **Risk Assessment:** ${riskLevel.toUpperCase()} ${riskType.toUpperCase()} RISK (${Math.round((risk.score || 0.8) * 100)}% score)\n\n🔍 **Why is risk high? (Drivers):**\n${driversList}\n\n💥 **Potential Impacts:**\n${impactText}${peakTime ? `\n\n⏱️ **Peak Window:** Expected around ${peakTime}` : ''}\n\n📋 **Recommended Action (${roleConfig.label}):**\n• ${recAction}\n• ${decision?.recommendations?.[1] || 'Stay updated via official advisories.'}${incidentNotice}`;
  }
  ...
  // Temporal parsing must win over persona-specific shortcuts below, otherwise a
  // "will it rain tomorrow?" question gets answered with today's conditions.
  if (language === 'english' && isTomorrowQuery) {
    if (isRainQuery) {
      return isRainyTomorrow
        ? `🌧️ **Yes, rain is expected in ${w.city} tomorrow!**\n\nTomorrow's forecast indicates **${tom.condition || 'Showers'}** with approximately ${tom.precipProb || 60}% chance of rain. Expected temperatures: High of ${tom.maxTemp}°C and Low of ${tom.minTemp}°C.\n\n${resolvedPersona === 'farmer' ? '🌾 **Farmer Advisory:** Hold off on irrigation tomorrow as natural precipitation is expected.' : '☂️ Remember to carry an umbrella or raincoat if stepping out tomorrow!'}`
        : `☀️ **No, rain is unlikely in ${w.city} tomorrow!**\n\nSkies are forecast to be largely **${tom.condition || 'Partly Cloudy / Clear'}** with only a ${tom.precipProb || 15}% precipitation probability. Temperature will reach a high of ${tom.maxTemp}°C and low of ${tom.minTemp}°C.\n\n${resolvedPersona === 'farmer' ? '🌾 **Farmer Advisory:** Conditions will be dry and suitable for scheduled irrigation or spraying.' : '👍 Good conditions for outdoor work and travel tomorrow!'}`;
    }
...
export function generateSmartLocalResponse(persona, language, weather, userQuery, forecast, extraContext = {}) {
  return formatNativeText(generateSmartLocalResponseRaw(persona, language, weather, userQuery, forecast, extraContext), language);
}
