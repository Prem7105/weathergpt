...
import {
  ...
  findHeavyRainEvent,
  fetchRiskAlertAssessment,
  buildRiskAlertPayload,
  getAlertConfig,
  isExpiredPushError,
} from '@/lib/notificationWeather';
import { shouldSendRiskAlert } from '@/lib/riskEngine';
  ...
  try {
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        if (subscription.severeWeatherEnabled) {
          const riskAlert = await fetchRiskAlertAssessment(subscription.location, subscription.persona || 'citizen', timezone);
          const existing = await WeatherAlertHistory.findOne({ subscriptionId: subscription._id, kind: 'risk-intelligence', locationKey }).sort({ sentAt: -1 });
          if (shouldSendRiskAlert(existing, riskAlert)) {
            const sent = await send(subscription, buildRiskAlertPayload(riskAlert, timezone));
            if (sent) {
              await WeatherAlertHistory.create({
                subscriptionId: subscription._id,
                eventId: riskAlert.eventId,
                kind: 'risk-intelligence',
                severityAmountMm: riskAlert.score,
                severityLevel: riskAlert.alertLevel,
                riskSignature: riskAlert.riskSignature,
                hazard: riskAlert.hazard,
                locationKey,
                eventEnd: riskAlert.expectedTime ? new Date(riskAlert.expectedTime) : null,
              });
            }
          }
        }
